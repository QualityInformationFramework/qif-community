...
def GetTolerances(qifDoc = QIFDocument.QIFDocumentType(), actfeatid = 0, tlabels = [""], talabels = [""], tdefs = [""], tadefs = [""]) :
    if (qifDoc.Characteristics is not None) & \
       ...
       (qifDoc.Results is not None) :
        measResults = qifDoc.Results.MeasurementResultsSet.MeasurementResults[0]
        if (measResults.MeasuredFeatures is not None) & \
           (measResults.MeasuredFeatures.FeatureMeasurement is not None) & \
           (measResults.MeasuredCharacteristics is not None) & \
           (measResults.MeasuredCharacteristics.CharacteristicMeasurements is not None) :
            # walk through actual characteristics to get characteristics applied to this actual feature actfeatid
            for charAct in measResults.MeasuredCharacteristics.CharacteristicMeasurements.CharacteristicMeasurement :
                if charAct.FeatureMeasurementIds is not None : # characteristics optionally point at features
                    for actFeatId in charAct.FeatureMeasurementIds.Id : # walk through feature references
                                        ...
                                        if charNom.id == charItem.CharacteristicNominalId.value() : # looking for the nominal referenced by the item
                                            for charDef in qifDoc.Characteristics.CharacteristicDefinitions.CharacteristicDefinition : # walk through characteristic definitions
                                                if charDef.id == charNom.CharacteristicDefinitionId.value() : # looking for the definition referenced by the nominal
                                                    ...
                                                    if (type(charAct) is QIFDocument.DiameterCharacteristicMeasurementType) & (type(charItem) is QIFDocument.DiameterCharacteristicItemType) & \
                                                       (type(charNom) is QIFDocument.DiameterCharacteristicNominalType) & (type(charDef) is QIFDocument.DiameterCharacteristicDefinitionType) :
                                                    ...
                                                    elif (type(charAct) is QIFDocument.FlatnessCharacteristicMeasurementType) & (type(charItem) is QIFDocument.FlatnessCharacteristicItemType) & \
                                                        ...
                                                        if flatAct.Value is not None : # do we have the optional actual value?
                                                            ...
                                                            if flatAct.Status.CharacteristicStatusEnum is not None : # we are only going to handle the enumeration
                                                                ...
                                                                tadefs[0] += ",OUTOL" # if we don't it's in, assume it's out
                                                            tadefs[0] += "\n"
                                                    elif (type(charAct) is QIFDocument.PerpendicularityCharacteristicMeasurementType) & (type(charItem) is QIFDocument.PerpendicularityCharacteristicItemType) & \
                                                    ...
                                                    elif (type(charAct) is QIFDocument.PositionCharacteristicMeasurementType) & (type(charItem) is QIFDocument.PositionCharacteristicItemType) & \
...
if (qifDoc.Features is not None) & \
   ...
   (qifDoc.Results is not None) :
    ...
    dmisoutput += "$$ Digital Metrology Standards Consortium (DMSC))\n"
    ...
    measResults = qifDoc.Results.MeasurementResultsSet.MeasurementResults[0]
    if (measResults.MeasuredFeatures is not None) & (measResults.MeasuredFeatures.FeatureMeasurement is not None) :
        # walk through the list of feature actuals (this is the main output loop)
        for featAct in measResults.MeasuredFeatures.FeatureMeasurement :
                            ...
                            if featNom.id == featItem.FeatureNominalId.value() :
                                ...
                                for featDef in qifDoc.Features.FeatureDefinitions.FeatureDefinition :
                                    ...
                                    if featDef.id == featNom.FeatureDefinitionId.value() :
                                        ...
                                        if (type(featAct) is QIFDocument.CircleFeatureMeasurementType) & (type(featItem) is QIFDocument.CircleFeatureItemType) & \
                                           (type(featNom) is QIFDocument.CircleFeatureNominalType) & (type(featDef) is QIFDocument.CircleFeatureDefinitionType) :
                                            ...
                                            dmisoutput += datdef[0]
                                        elif (type(featAct) is QIFDocument.CylinderFeatureMeasurementType) & (type(featItem) is QIFDocument.CylinderFeatureItemType) & \
                                            ...
                                            dmisoutput += datdef[0]
                                        elif (type(featAct) is QIFDocument.PlaneFeatureMeasurementType) & (type(featItem) is QIFDocument.PlaneFeatureItemType) & \
