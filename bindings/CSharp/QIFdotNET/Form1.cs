...
namespace QifStructureReader1
{
	public partial class Form1 : Form
	{
		...
		private void GetTolerances(QIFDocumentType qifDoc, long actfeatid, ref string tlabels, ref string talabels, ref string tdefs, ref string tadefs)
		{
			...
			var measResults = qifDoc.Results.MeasurementResultsSet.MeasurementResults[0];
			if (measResults.MeasuredFeatures == null || // make sure we have both (optional) actual features and characteristics
				...
				measResults.MeasuredCharacteristics.CharacteristicMeasurements == null)
				return;

			// walk through actual characteristics to get characteristics applied to this actual feature actfeatid
			for (int i = 0; i < measResults.MeasuredCharacteristics.CharacteristicMeasurements.CharacteristicMeasurement.Length; i++)
			{
				var characteristicMeasurement = measResults.MeasuredCharacteristics.CharacteristicMeasurements.CharacteristicMeasurement[i];
				if (characteristicMeasurement.FeatureMeasurementIds == null) // characteristics optionally point at features
					continue;
				for (int j = 0; j < characteristicMeasurement.FeatureMeasurementIds.Id.Length; j++) // walk through feature references
				{
					if (characteristicMeasurement.FeatureMeasurementIds.Id[j].Value != actfeatid) // look for a match
						continue;
					...
					for (int k = 0; k < qifDoc.Characteristics.CharacteristicItems.CharacteristicItem.Length; k++) // walk through characteristic items
					{
						...
						if (characteristicItem.id != characteristicMeasurement.CharacteristicItemId.Value) // looking for the item referenced by the actual
							continue;
						...
						for (int m = 0; m < qifDoc.Characteristics.CharacteristicNominals.CharacteristicNominal.Length; m++) // walk through characteristic nominals
						{
							...
							for (int n = 0; n < qifDoc.Characteristics.CharacteristicDefinitions.CharacteristicDefinition.Length; n++) // walk through characteristic definitions
							{
								...
								if (characteristicMeasurement is DiameterCharacteristicMeasurementType &&
								...
								{
									var diaAct = (DiameterCharacteristicMeasurementType)Convert.ChangeType(characteristicMeasurement, characteristicMeasurement.GetType());
								...
								else if (characteristicMeasurement is FlatnessCharacteristicMeasurementType &&
								...
								{
									var flatAct = (FlatnessCharacteristicMeasurementType)Convert.ChangeType(characteristicMeasurement, characteristicMeasurement.GetType());
								...
								else if (characteristicMeasurement is PerpendicularityCharacteristicMeasurementType &&
								...
								{
									var perpAct = (PerpendicularityCharacteristicMeasurementType)Convert.ChangeType(characteristicMeasurement, characteristicMeasurement.GetType());
								...
								else if (characteristicMeasurement is PositionCharacteristicMeasurementType &&
								...
								{
									var posAct = (PositionCharacteristicMeasurementType)Convert.ChangeType(characteristicMeasurement, characteristicMeasurement.GetType());
		...
		private void GetPositionTolerance(QIFDocumentType qifDoc,
			...
			ref string tdefs,
			ref string tadefs,
			PositionCharacteristicMeasurementType posAct,
		...
		private void GetPerpendicularityTolerance(QIFDocumentType qifDoc,
			...
			ref string tdefs,
			ref string tadefs,
			PerpendicularityCharacteristicMeasurementType perpAct,
		...
		private static void GetFlatnessTolerance(ref string tlabels,
			...
			ref string tdefs,
			ref string tadefs,
			FlatnessCharacteristicMeasurementType flatAct,
...
#region read QIF
		...
		$$   C:\Python34\pyxb-next\QIF30\QIF30\cpp.qif
			...
			try
			{
				...
				if (openFileDialog1.ShowDialog() == DialogResult.OK)
				{
					...
					if (qifDoc.Features != null &&
						...
						qifDoc.Results != null &&
						qifDoc.Results.MeasurementResultsSet != null &&
						...
						qifDoc.Results.MeasurementResultsSet.MeasurementResults.Length > 0)
					{
						...
						MeasurementResultsType measResults = qifDoc.Results.MeasurementResultsSet.MeasurementResults[0];
						if (measResults.MeasuredFeatures != null)
						{
							// walk through the list of feature actuals (this is the main output loop)
							for (int i = 0; i < measResults.MeasuredFeatures.FeatureMeasurement.Length; i++)
							{
								...
								for (int j = 0; j < qifDoc.Features.FeatureItems.FeatureItem.Length; j++)
								{
									...
									if (qifDoc.Features.FeatureItems.FeatureItem[j].id == measResults.MeasuredFeatures.FeatureMeasurement[i].FeatureItemId.Value)
									{
										// walk through the list of feature nominals...
										for (int k = 0; k < qifDoc.Features.FeatureNominals.FeatureNominal.Length; k++)
										{
											// ...to find the associated feature nominal (the item->nominal link is required in QIF 3.0)
											if (qifDoc.Features.FeatureNominals.FeatureNominal[k].id == qifDoc.Features.FeatureItems.FeatureItem[j].FeatureNominalId.Value)
											{
												...
												for (int n = 0; n < qifDoc.Features.FeatureDefinitions.FeatureDefinition.Length; n++)
												{
													...
													if (qifDoc.Features.FeatureDefinitions.FeatureDefinition[n].id == qifDoc.Features.FeatureNominals.FeatureNominal[k].FeatureDefinitionId.Value)
													{
														...
														if (measResults.MeasuredFeatures.FeatureMeasurement[i] is CircleFeatureMeasurementType &&
														   ...
														   qifDoc.Features.FeatureDefinitions.FeatureDefinition[n] is CircleFeatureDefinitionType)
														{
															...
															CircleFeatureMeasurementType circAct = (CircleFeatureMeasurementType)Convert.ChangeType(measResults.MeasuredFeatures.FeatureMeasurement[i],
														   ...
														   qifDoc.Features.FeatureDefinitions.FeatureDefinition[n] is CylinderFeatureDefinitionType)
														{
															...
															CylinderFeatureDefinitionType cylDef = (CylinderFeatureDefinitionType)Convert.ChangeType(qifDoc.Features.FeatureDefinitions.FeatureDefinition[n],
																qifDoc.Features.FeatureDefinitions.FeatureDefinition[n].GetType());
															CylinderFeatureMeasurementType cylAct = (CylinderFeatureMeasurementType)Convert.ChangeType(measResults.MeasuredFeatures.FeatureMeasurement[i],
																measResults.MeasuredFeatures.FeatureMeasurement[i].GetType());
														...
														else if (measResults.MeasuredFeatures.FeatureMeasurement[i] is PlaneFeatureMeasurementType &&
																 ...
																 qifDoc.Features.FeatureDefinitions.FeatureDefinition[n] is PlaneFeatureDefinitionType)
														{
															PlaneFeatureNominalType planNom = (PlaneFeatureNominalType)Convert.ChangeType(qifDoc.Features.FeatureNominals.FeatureNominal[k],
																qifDoc.Features.FeatureNominals.FeatureNominal[k].GetType());
															PlaneFeatureMeasurementType planAct = (PlaneFeatureMeasurementType)Convert.ChangeType(measResults.MeasuredFeatures.FeatureMeasurement[i],
																measResults.MeasuredFeatures.FeatureMeasurement[i].GetType());
															...
															if (distDef != null)
															{
																...
																if (distDef.Item.GetType().Name == "LinearToleranceType")
																{
																	...
																	if (distTol != null)
																	{
																		...
																		{
																			if (distTol.ItemsElementName[t] == ItemsChoiceType6.MaxValue)
																			{
																			...
																			else if (distTol.ItemsElementName[t] == ItemsChoiceType6.MinValue)
																			{
		...
		*/
		private void QifDocumentWriteCall()
		{
			try
			{
				...
				List<FeatureMeasurementBaseType> featActs = new List<FeatureMeasurementBaseType>();
				...
				List<CharacteristicMeasurementBaseType> charActs = new List<CharacteristicMeasurementBaseType>();
				#endregion
				...
				PlaneMeasurementDeterminationType planAActDet = new PlaneMeasurementDeterminationType();
				...
				PlaneFeatureMeasurementType planAAct = new PlaneFeatureMeasurementType();
				...
				// actual location
				planAAct.Location = new MeasuredPointType();
				...
				// plane normal
				planAAct.Normal = new MeasuredUnitVectorType();
				...
				flatItem.FeatureItemIds = new ArrayReferenceType();
				...
				flatItem.MeasurementDeviceIds = new ArrayReferenceType();
				...
				FlatnessCharacteristicMeasurementType flatAct = new FlatnessCharacteristicMeasurementType();
				...
				// actual value
				MeasuredLinearValueType flatActVal = new MeasuredLinearValueType();
				...
				flatAct.FeatureMeasurementIds = new ArrayReferenceType();
				flatAct.FeatureMeasurementIds.n = 1;
				flatAct.FeatureMeasurementIds.Id = new QIFReferenceFullType[1];
				flatAct.FeatureMeasurementIds.Id[0] = planAActRef;

				// add our measurement device (this is the device that actually measured the characteristic)
				flatAct.MeasurementDeviceIds = new ArrayReferenceType();
				...
				CylinderMeasurementDeterminationType cylBActDet = new CylinderMeasurementDeterminationType();
				...
				CylinderFeatureMeasurementType cylBAct = new CylinderFeatureMeasurementType();
				...
				// cylinder location
				MeasuredPointType acylBcen = new MeasuredPointType();
				...
				// cylinder axis
				MeasuredUnitVectorType acylBvec = new MeasuredUnitVectorType();
				...
				// cylinder axis
				MeasuredAxisType acylBaxis = new MeasuredAxisType();
				acylBaxis.AxisPoint = acylBcen;
				acylBaxis.Direction = acylBvec;
				cylBAct.Axis = acylBaxis;

				// cylinder diameter
				MeasuredLinearValueType acylBDiam = new MeasuredLinearValueType();
				...
				diaBCTol.ItemsElementName = new ItemsChoiceType6[2];
				diaBCTol.Items[0] = plusTolBC;
				diaBCTol.ItemsElementName[0] = ItemsChoiceType6.MaxValue;
				diaBCTol.Items[1] = minusTolBC;
				diaBCTol.ItemsElementName[1] = ItemsChoiceType6.MinValue;
				...
				DiameterCharacteristicMeasurementType diaBAct = new DiameterCharacteristicMeasurementType();
				...
				// actual value
				MeasuredLinearValueType diaBActVal = new MeasuredLinearValueType();
				...
				diaBAct.FeatureMeasurementIds = new ArrayReferenceType();
				diaBAct.FeatureMeasurementIds.n = 1;
				diaBAct.FeatureMeasurementIds.Id = new QIFReferenceFullType[1];
				diaBAct.FeatureMeasurementIds.Id[0] = cylBActRef;

				// add our measurement device (this is the device that actually measured the characteristic)
				diaBAct.MeasurementDeviceIds = new ArrayReferenceType();
				...
				// point tolerance at feature
				perpItem.FeatureItemIds = new ArrayReferenceType();
				...
				// add our measurement devices (this is a list of devices that could measure the characteristic)
				perpItem.MeasurementDeviceIds = new ArrayReferenceType();
				...
				PerpendicularityCharacteristicMeasurementType perpAct = new PerpendicularityCharacteristicMeasurementType();
				...
				// actual value
				MeasuredLinearValueType perpActVal = new MeasuredLinearValueType();
				perpActVal.Value = 0.07M;
				perpAct.Value = perpActVal;

				// point actual tolerance at actual feature
				perpAct.FeatureMeasurementIds = new ArrayReferenceType();
				perpAct.FeatureMeasurementIds.n = 1;
				perpAct.FeatureMeasurementIds.Id = new QIFReferenceFullType[1];
				perpAct.FeatureMeasurementIds.Id[0] = cylBActRef;

				// add our measurement device (this is the device that actually measured the characteristic)
				perpAct.MeasurementDeviceIds = new ArrayReferenceType();
				...
				CylinderMeasurementDeterminationType cylCActDet = new CylinderMeasurementDeterminationType();
				...
				// make a cylinder feature actual
				CylinderFeatureMeasurementType cylCAct = new CylinderFeatureMeasurementType();
				...
				// cylinder location
				MeasuredPointType acylCcen = new MeasuredPointType();
				...
				// cylinder axis
				MeasuredUnitVectorType acylCvec = new MeasuredUnitVectorType();
				...
				// cylinder axis
				MeasuredAxisType acylCaxis = new MeasuredAxisType();
				acylCaxis.AxisPoint = acylCcen;
				acylCaxis.Direction = acylCvec;
				cylCAct.Axis = acylCaxis;

				// cylinder diameter
				MeasuredLinearValueType acylCDiam = new MeasuredLinearValueType();
				...
				// make a diameter characteristic actual
				DiameterCharacteristicMeasurementType diaCAct = new DiameterCharacteristicMeasurementType();
				...
				// actual value
				MeasuredLinearValueType diaCActVal = new MeasuredLinearValueType();
				...
				diaCAct.FeatureMeasurementIds = new ArrayReferenceType();
				diaCAct.FeatureMeasurementIds.n = 1;
				diaCAct.FeatureMeasurementIds.Id = new QIFReferenceFullType[1];
				diaCAct.FeatureMeasurementIds.Id[0] = cylCActRef;

				// add our measurement device (this is the device that actually measured the characteristic)
				diaCAct.MeasurementDeviceIds = new ArrayReferenceType();
				...
				// point tolerance at feature
				posCItem.FeatureItemIds = new ArrayReferenceType();
				...
				// add our measurement devices (this is a list of devices that could measure the characteristic)
				posCItem.MeasurementDeviceIds = new ArrayReferenceType();
				...
				PositionCharacteristicMeasurementType posCAct = new PositionCharacteristicMeasurementType();
				...
				// actual value
				MeasuredLinearValueType posCActVal = new MeasuredLinearValueType();
				posCActVal.Value = 0.102M;
				posCAct.Value = posCActVal;

				// point actual tolerance at actual feature
				posCAct.FeatureMeasurementIds = new ArrayReferenceType();
				posCAct.FeatureMeasurementIds.n = 1;
				posCAct.FeatureMeasurementIds.Id = new QIFReferenceFullType[1];
				posCAct.FeatureMeasurementIds.Id[0] = cylCActRef;

				// add our measurement device (this is the device that actually measured the characteristic)
				posCAct.MeasurementDeviceIds = new ArrayReferenceType();
				...
				CircleMeasurementDeterminationType circActDet = new CircleMeasurementDeterminationType();
				...
				CircleFeatureMeasurementType circAct = new CircleFeatureMeasurementType();
				...
				// circle location
				circAct.Location = new MeasuredPointType();
				...
				// circle normal
				circAct.Normal = new MeasuredUnitVectorType();
				...
				// feature size
				MeasuredLinearValueType adiaSize = new MeasuredLinearValueType();
				adiaSize.Value = 6.2M;
				...
				diaTol.ItemsElementName = new ItemsChoiceType6[2];
				diaTol.Items[0] = highSize;
				diaTol.ItemsElementName[0] = ItemsChoiceType6.MaxValue;
				diaTol.Items[1] = lowSize;
				diaTol.ItemsElementName[1] = ItemsChoiceType6.MinValue;
				diaDef.Item = diaTol;
				...
				// make a diameter characteristic actual
				DiameterCharacteristicMeasurementType diaAct = new DiameterCharacteristicMeasurementType();
				...
				// actual value
				MeasuredLinearValueType diaActVal = new MeasuredLinearValueType();
				...
				diaAct.FeatureMeasurementIds = new ArrayReferenceType();
				diaAct.FeatureMeasurementIds.n = 1;
				diaAct.FeatureMeasurementIds.Id = new QIFReferenceFullType[1];
				diaAct.FeatureMeasurementIds.Id[0] = circActRef;

				// add our measurement device (this is the device that actually measured the characteristic)
				diaAct.MeasurementDeviceIds = new ArrayReferenceType();
				...
				// point tolerance at feature
				posItem.FeatureItemIds = new ArrayReferenceType();
				...
				// add our measurement devices (this is a list of devices that could measure the characteristic)
				posItem.MeasurementDeviceIds = new ArrayReferenceType();
				...
				// make a position characteristic actual
				PositionCharacteristicMeasurementType posAct = new PositionCharacteristicMeasurementType();
				...
				// actual value
				MeasuredLinearValueType posActVal = new MeasuredLinearValueType();
				posActVal.Value = 0.0025M;
				posAct.Value = posActVal;

				// point actual tolerance at actual feature
				posAct.FeatureMeasurementIds = new ArrayReferenceType();
				posAct.FeatureMeasurementIds.n = 1;
				posAct.FeatureMeasurementIds.Id = new QIFReferenceFullType[1];
				posAct.FeatureMeasurementIds.Id[0] = circActRef;

				// add our measurement device (this is the device that actually measured the characteristic)
				posAct.MeasurementDeviceIds = new ArrayReferenceType();
				...
				formalStandard.Organization = new StandardsOrganizationType();
				formalStandard.Organization.Item = StandardsOrganizationEnumType.ASME;
				formalStandard.Designator = "Y14.5";
				formalStandard.Year = "2009";
				formalStandard.id = qifid++; // required id

				// add standard to document
				qifdoc.StandardsDefinitions = new StandardsType();
				qifdoc.StandardsDefinitions.n = 1;
				qifdoc.StandardsDefinitions.Standard = new StandardType[1];
				qifdoc.StandardsDefinitions.Standard[0] = formalStandard;

				// characteristics reference standard
				QIFReferenceType standardRef = new QIFReferenceType();
				standardRef.Value = formalStandard.id;
				qifdoc.Characteristics.FormalStandardId = standardRef;
				...
				// measured features
				measResults.MeasuredFeatures = new FeatureMeasurementsType();
				measResults.MeasuredFeatures.n = (uint)featActs.Count();
				measResults.MeasuredFeatures.FeatureMeasurement = new FeatureMeasurementBaseType[featActs.Count()];
				for (int i = 0; i < featActs.Count(); i++) measResults.MeasuredFeatures.FeatureMeasurement[i] = featActs[i];

				// measured characteristics
				measResults.MeasuredCharacteristics = new MeasuredCharacteristicsType();
				measResults.MeasuredCharacteristics.CharacteristicMeasurements = new CharacteristicMeasurementsType();
				measResults.MeasuredCharacteristics.CharacteristicMeasurements.n = (uint)charActs.Count();
				measResults.MeasuredCharacteristics.CharacteristicMeasurements.CharacteristicMeasurement = new CharacteristicMeasurementBaseType[charActs.Count()];
				for (int i = 0; i < charActs.Count(); i++) measResults.MeasuredCharacteristics.CharacteristicMeasurements.CharacteristicMeasurement[i] = charActs[i];

				// add measured results to list of measured results
				qifdoc.Results = new ResultsType();
				qifdoc.Results.MeasurementResultsSet = new MeasurementResultsSetType();
				qifdoc.Results.MeasurementResultsSet.n = 1;
				qifdoc.Results.MeasurementResultsSet.MeasurementResults = new MeasurementResultsType[1];
				qifdoc.Results.MeasurementResultsSet.MeasurementResults[0] = measResults;
				...
				qifdoc.Plan = new PlanType();
				qifdoc.Plan.PlanRoot = planRoot;
