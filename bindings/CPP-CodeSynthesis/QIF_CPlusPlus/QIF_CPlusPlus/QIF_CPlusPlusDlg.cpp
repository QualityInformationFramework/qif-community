...
void CQIF_CPlusPlusDlg::GetTolerances(QIFDocumentType &qifDoc, long actfeatid, CString &tlabels, CString &talabels, CString &tdefs, CString &tadefs)
{
	...
	if (qifDoc.Characteristics() != NULL && // make sure we have(optional) characteristics and measurements
		...
		qifDoc.Results() != NULL &&
		qifDoc.Results()->MeasurementResultsSet().MeasurementResults().size() > 0)
	{
			  ...
			  measResults.MeasuredCharacteristics() != NULL)
		{
			...
			for (int i = 0; i < measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement().size(); i++)
			{
				if(measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement()[i].FeatureMeasurementIds() != NULL) // characteristics optionally point at features
				{
					for (int j = 0; j < measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement()[i].FeatureMeasurementIds()->Id().size(); j++) // walk through feature references
					{
						if (measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement()[i].FeatureMeasurementIds()->Id()[j] == actfeatid) // look for a match
						{
							...
							{
								if (qifDoc.Characteristics()->CharacteristicItems()->CharacteristicItem()[k].id() == measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement()[i].CharacteristicItemId()) // looking for the item referenced by the actual
								{
										...
										if (qifDoc.Characteristics()->CharacteristicItems()->CharacteristicItem()[k].CharacteristicNominalId() == qifDoc.Characteristics()->CharacteristicNominals().CharacteristicNominal()[m].id()) // looking for the nominal referenced by the item
										{
											for (int n = 0; n < qifDoc.Characteristics()->CharacteristicDefinitions().CharacteristicDefinition().size(); n++) // walk through characteristic definitions
											{
												if (qifDoc.Characteristics()->CharacteristicNominals().CharacteristicNominal()[m].CharacteristicDefinitionId() == qifDoc.Characteristics()->CharacteristicDefinitions().CharacteristicDefinition()[n].id()) // looking for the definition referenced by the nominal
												{
													...
													if(DiameterCharacteristicMeasurementType *diaAct = dynamic_cast<DiameterCharacteristicMeasurementType *> (&(measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement()[i])))
													{
													...
													else if(FlatnessCharacteristicMeasurementType *flatAct = dynamic_cast<FlatnessCharacteristicMeasurementType *> (&(measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement()[i])))
													{
													...
													if(PerpendicularityCharacteristicMeasurementType *perpAct = dynamic_cast<PerpendicularityCharacteristicMeasurementType *> (&(measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement()[i])))
													{
													...
													else if(PositionCharacteristicMeasurementType *posAct = dynamic_cast<PositionCharacteristicMeasurementType *> (&(measResults.MeasuredCharacteristics()->CharacteristicMeasurements().CharacteristicMeasurement()[i])))
													{
...
void CQIF_CPlusPlusDlg::OnBnClickedReadqif()
{
			...
			qifDoc->Results() != NULL)
	{
		...
		dmisoutput += _T("$$ Digital Metrology Standards Consortium (DMSC))\n");
		...
		if (measResults.MeasuredFeatures() != NULL)
		{
			...
			for (int i = 0; i < measResults.MeasuredFeatures()->FeatureMeasurement().size(); i++)
			{
				...
				for (int j = 0; j < qifDoc->Features()->FeatureItems()->FeatureItem().size(); j++)
				{
					...
					if (qifDoc->Features()->FeatureItems()->FeatureItem()[j].id() == *measResults.MeasuredFeatures()->FeatureMeasurement()[i].FeatureItemId())
					{
								  ...
								  qifDoc->Features()->FeatureNominals()->FeatureNominal()[k].id() == qifDoc->Features()->FeatureItems()->FeatureItem()[j].FeatureNominalId())
							{
								...
								for (int n = 0; n < qifDoc->Features()->FeatureDefinitions()->FeatureDefinition().size(); n++)
								{
									...
									if (qifDoc->Features()->FeatureDefinitions()->FeatureDefinition()[n].id() == qifDoc->Features()->FeatureNominals()->FeatureNominal()[k].FeatureDefinitionId())
									{
										...
										if(CircleFeatureMeasurementType *circAct = dynamic_cast<CircleFeatureMeasurementType *> (&(measResults.MeasuredFeatures()->FeatureMeasurement()[i])))
										{
										...
										else if(CylinderFeatureMeasurementType *cylAct = dynamic_cast<CylinderFeatureMeasurementType *> (&(measResults.MeasuredFeatures()->FeatureMeasurement()[i])))
										{
										...
										else if(PlaneFeatureMeasurementType *planAct = dynamic_cast<PlaneFeatureMeasurementType *> (&(measResults.MeasuredFeatures()->FeatureMeasurement()[i])))
										{
...
#pragma region Datum A plane
	...
	PlaneMeasurementDeterminationType planAActDet;
	...
	PlaneFeatureMeasurementType planAAct;
	...
	// plane normal
	MeasuredUnitVectorType planAActNor;
	...
	// add to list
	featActs.FeatureMeasurement().push_back(planAAct);
	...
	// actual value
	MeasuredLinearValueType flatActVal(0.023);
	flatAct.Value(flatActVal);

	// point actual tolerance at actual feature
	QIFReferenceType planAActRef(planAAct.id());
	ArrayReferenceType flatActFeatIds;
	...
	ArrayReferenceType flatActDevIds;
	flatActDevIds.Id().push_back(cmm2Ref);
	flatActDevIds.n(1);
	flatAct.MeasurementDeviceIds(flatActDevIds);

	// add to list
	charActs.CharacteristicMeasurement().push_back(flatAct);
...
#pragma region datum B cylinder
	...
	CylinderMeasurementDeterminationType cylBActDet;
	...
	CylinderFeatureMeasurementType cylBAct;
	...
	DiameterCharacteristicMeasurementType diaBAct;
	...
	// actual value
	MeasuredLinearValueType diaBActVal(12.699);
	diaBAct.Value(diaBActVal);

	// point actual tolerance at actual feature
	QIFReferenceFullType cylBActRef(cylBAct.id());
	ArrayReferenceType diaBFeatActIds;
	diaBFeatActIds.n(1);
	diaBFeatActIds.Id().push_back(cylBActRef);
	diaBAct.FeatureMeasurementIds(diaBFeatActIds);

	// add our measurement device (this is the device that actually measured the characteristic)
	ArrayReferenceType diaBActDevIds;
	diaBActDevIds.n(1);
	diaBActDevIds.Id().push_back(cmm2Ref);
	diaBAct.MeasurementDeviceIds(diaBActDevIds);

	// add to list
	charActs.CharacteristicMeasurement().push_back(diaBAct);
	charActs.n(charActs.n() + 1); // increment the n counter
...
#pragma region datum C cylinder
	...
	CylinderMeasurementDeterminationType cylCActDet;
	...
	// make a cylinder feature actual
	CylinderFeatureMeasurementType cylCAct;
	...
	// make a diameter characteristic actual
	DiameterCharacteristicMeasurementType diaCAct;
	...
	PositionCharacteristicMeasurementType posCAct;
...
#pragma region circle back to ABC
	...
	CircleMeasurementDeterminationType circActDet;
	...
	CircleFeatureMeasurementType circAct;
	...
	// circle normal
	MeasuredUnitVectorType circActNor;
	...
	// add to list
	featActs.FeatureMeasurement().push_back(circAct);
	featActs.n(featActs.n() + 1); // increment the n counter
...
#pragma region QIF document
	...
	// add some application identifying information
	xsd::qif30::ApplicationType app; // there is a namespace conflict with this type so it must be prefixed with xsd::qif3::
	app.Name(_T("QIF_CPlusPlus open source QIF application"));
	app.Organization(_T("Digital Metrology Standards Consortium (DMSC)"));
	...
	organization.StandardsOrganizationEnum(StandardsOrganizationEnumType::ASME);
	StandardType formalStandard;
	formalStandard.Organization(organization);
	formalStandard.Designator(_T("Y14.5"));
	formalStandard.Year(_T("2009"));
	formalStandard.id(qifid++); // required id

	// add standard to document
	StandardsType standards;
	standards.n(1);
	standards.Standard().push_back(formalStandard);
	qifdoc.StandardsDefinitions(standards);

	// point characteristics at standard
	QIFReferenceType standardRef(formalStandard.id());
	charAspects.FormalStandardId(standardRef);
	...
	measuredCharacteristics.CharacteristicMeasurements(charActs);
	...
	qifdoc.Results(measurementsResults);
	...
	map[_T("")].name = _T("http://qifstandards.org/xsd/qif3");
